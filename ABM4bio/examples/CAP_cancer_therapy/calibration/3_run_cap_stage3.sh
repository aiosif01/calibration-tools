...
# Usage: ./calibration/3_run_cap_stage3.sh [n_trials] [seed]
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
source "$SCRIPT_DIR/common.sh"

TRIALS="${1:-80}"
SEED="${2:-1234}"
LOCK_FILE="$(cap_lock_file stage3)"

check_runtime
cd "$CAP_ROOT"

if [[ -e "$LOCK_FILE" ]]; then
  echo "ERROR: stage3 calibration active or stale lock exists: $LOCK_FILE" >&2
  exit 10
fi

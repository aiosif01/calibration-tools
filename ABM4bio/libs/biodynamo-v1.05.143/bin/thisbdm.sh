...
_source_thisbdm()
{
  ...
  if [ "$with_pyenv" = 'ON' ]; then
    if [ -n "${old_bdmsys}" ] ; then
      if [ -n "${PATH}" ]; then
        ...
        PATH=$_newpath
        _drop_bdm_from_path "$PATH" "$PYENV_ROOT/versions/3.9.1/bin"
        PATH=$_newpath
  ...
  if [ "$with_pyenv" = 'ON' ]; then
    ...
    export LD_LIBRARY_PATH="$PYENV_ROOT/versions/3.9.1/lib":$LD_LIBRARY_PATH
  fi
  ########

...
function source_thisbdm
    ...
    if test "$with_pyenv" = 'ON'
        if test -n "$old_bdmsys"
            if test -n "$PATH"
                ...
                _drop_from_var PATH "$PYENV_ROOT/versions/3.9.1/bin"
    ...
    if test "$with_pyenv" = 'ON'
        ...
        pyenv shell 3.9.1; or return 1

        # Location of jupyter executable (installed with `pip install` command)
        set -pgx PATH "$PYENV_ROOT/versions/3.9.1/bin"
        set -pgx LD_LIBRARY_PATH "$PYENV_ROOT/versions/3.9.1/lib"
    end
    ########

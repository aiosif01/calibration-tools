function [a,redX2,sigma_a,sigma_y,corr_a,R_sq,cvg_hst] = lm(func,a,t,y_dat,weight,da,a_lb,a_ub,opts)
%
% Levenberg Marquardt curve-fitting: minimize sum of weighted squared residuals
% ----------  INPUT  VARIABLES  -----------
% func   = function of n independent variables, 't', and m coefficients, 'a', 
%          returning the simulated model: y_hat = func(t,a)
...
%if ( tensor_coefficient && prnt == 3 ) prnt = 2; end
 ...
 y_init = feval(func,t,a);         % residual error using a_try
...
[JtWJ,JtWdy,X2,y_hat,J] = lm_matx(func,t,a_old,y_old,1,J,a,y_dat,weight,da);
...
cvg_hst = ones(MaxEvals,Ncof+3);

while ( ~stop && func_calls <= MaxEvals ) % --- Start of Main Loop
...
%  big = max(abs(h./p)) > 2;                      % this is a big step
   ...
   a_try = a + h(idx);
   % apply bounds
   a_try = min(max(a_lb,a_try),a_ub);
   % residual error using a_try
   delta_y = y_dat - feval(func,t,a_try);
   % floating point error, so do a break
   if ~all(isfinite(delta_y))
     stop = 1;
     break     
   end
   ...
   if ( rho > epsilon_4 )                         % it IS significantly better
     ...
     [JtWJ,JtWdy,X2,y_hat,J] = lm_matx(func,t,a_old,y_old,dX2,J,a,y_dat,weight,da);
     ...
     if ( ~rem(iteration,2*Ncof) )
       [JtWJ,JtWdy,dX2,y_hat,J] = lm_matx(func,t,a_old,y_old,-1,J,a,y_dat,weight,da);
     end
...
function J = lm_FD_J(func,t,a,y,da)
%
...
% y  = func(t,a) initialised by user before each call to lm_FD_J    (m x 1)
% da = fractional increment of a for numerical derivatives
%      da(j)>0 central differences calculated
...
%   Press, et al., Numerical Recipes, Cambridge Univ. Press, 1992, Chapter 15.
 ...
 as=a; J=zeros(m,n); del=zeros(n,1);         
 % START --- loop over all coefficients
 for j = 1 : n                 
   % coefficient perturbation
   del(j) = da(j) * (1+abs(a(j)));   
   % perturb coefficient a(j)
   a(j) = as(j) + del(j);          
...
J = J + ( y - y_old - J*h ) * h' / (h'*h); 

% endfunction # ---------------------------------------------- LM_Broyden_J



function [JtWJ,JtWdy,Chi_sq,y_hat,J] = lm_matx(func,t,a_old,y_old,dX2,J,a,y_dat,weight,da)
%
...
% a      = current  coefficient values                                     (n x 1)
% y_dat  = data to be fit by func(t,a)                                     (m x 1)
...
% evaluate model using coefficients 'a'
y_hat = feval(func,t,a);            

func_calls = func_calls + 1;

if ( ~rem(iteration,2*Ncof) || dX2 > 0 ) 
    % finite difference
    J = lm_FD_J(func,t,a,y_hat,da); 
else
    % rank-1 update
    J = lm_Broyden_J(a_old,y_old,J,a,y_hat);
end

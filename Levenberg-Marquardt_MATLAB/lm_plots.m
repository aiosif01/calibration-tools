...
print(sprintf('%sA.pdf', filename),'-dpdf');
...
hc99 = patch('XData',tp, 'YData',yp99, 'FaceColor', patchColor99, 'EdgeColor', patchColor99);
hc95 = patch('XData',tp, 'YData',yp95, 'FaceColor', patchColor95, 'EdgeColor', patchColor95);
hd   = plot(t,y_dat,'ob'); 
hf   = plot(t,y_fit,'-k');
hold off
axis('tight')
...
xlabel('t'), ylabel('y(t)')
print(sprintf('%sB.pdf', filename),'-dpdf');
...
axis('tight')
xlabel('y_{data} - y_{fit}'), ylabel('count')
print(sprintf('%sC.pdf', filename),'-dpdf');
